...
export class ProductosComponent implements OnInit {
  ...
  validar(): number {
    ...
    if (this.model.descripcion.length > 45) {
      this.error.descripcion.error = true;
  ...
  guardar() {
    if (this.validar() == 0) {
      ...
      if (this.edit == false) {
        ...
        this.rest.put('api/productos/' + this.model.referencia, this.model).then(res => {
          this.modal.showAlert('Información almacenada satisfactoriamente', 2);
          this.cargarGrid();
          this.modal.hideLoading();
          this.cancelar();
  ...
  eliminar() {
    ...
    if (data === null) {
      ...
      this.rest.delete('api/productos/' + data.referencia).then(res => {
  ...
  enviarCarro(referencia: string, valor: number) {
    this.rest.post('api/carros', { referenciaProducto: referencia, cantidad: 1, total: valor, estado: 1, idUsuario: this.auth.idUser }).then(res => {
